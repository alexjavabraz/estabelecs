...
public class SimpleJDBCDaoImpl extends GenericJdbcDao implements SimpleJDBCDao {
	...
	public List<Estabelecimento> pesquisarEstabelecimentos(String idCategoria, String idGrupo, String idSubGrupo,
			String query) {
	...
	private List<Estabelecimento> preencheDTO(List<Estabelecimento> arrretorno, ResultSet rs) {
		...
		try {
			while (rs.next()) {
				...
				retorno.setNomeFantasia(rs.getString("nome_fantasia")); // "Vedhanta
																		// Galeteria";
				retorno.setBairro(rs.getString("bairro")); // "Granja Viana";
				// retorno.setCep() = "Cotia";
				// retorno.ehDestaque = (rs.getString("id_destaque") != null);
				// // Boolean.TRUE;
				retorno.setEmail(rs.getString("email")); // "contato@vedhanta.com.br";
				retorno.setCep(rs.getString("cep")); // "06708-645";
				retorno.setEndRua(rs.getString("end_rua") + " ," + rs.getString("end_numero")); // "Rua
																								// Jose
																								// Félix
																								// de
																								// Oliveira,
				...
				retorno.setTelefone(rs.getString("telefone")); // "(11)
																// 4702-2618";
				retorno.setSlogam(rs.getString("slogam")); // "Cardápio variado,
															// iguarias por
															// preços acessíveis
															// e galetos
															// irrestíveis.";
	...
	public List<Estabelecimento> listarPorGrupo(Integer idGrupo) {
		...
		try {
			...
			String sql = " SELECT e.id, e.nome_fantasia, e.end_rua, e.end_numero, e.end_complemento, e.bairro,  "+ 
					" e.slogam, e.imagem1, e.imagem2, e.imagem3, e.imagem4, e.telefone, e.site, e.email, e.bairro, e.facebook,e.id_destaque,  "+ 	
					" e.cep, s.id_sg, s.nome_sub_grupo, e.dh_inclusao   "+
			...
			if(mapaRetorno != null && !mapaRetorno.isEmpty()){
				for(java.util.Map<String, Object> linha : mapaRetorno){
					Estabelecimento e = new Estabelecimento();
					e.setNomeFantasia((String) linha.get("nome_fantasia"));
					e.setId((Integer) linha.get("id"));
					e.setCep((String) linha.get("cep"));
					e.setBairro((String) linha.get("bairro"));
					e.setEndRua((String) linha.get("end_rua"));
					e.setEndNumero((String)linha.get("end_numero"));
					e.setEmail((String) linha.get("email"));
					e.setSite((String) linha.get("site"));
					e.setFacebook((String) linha.get("facebook"));
